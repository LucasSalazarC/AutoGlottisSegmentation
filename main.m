...
graySourceList = {'red', 'blue', 'green', 'rgb2gray'};
...
for k = 1:length(graySourceList)
    ...
    for j = 1:length(vidList)
        ...
        tic
        [outputContours,vidSize] =  Segmentation(vidName, vidPath, frames, FDmatrix, gndhisto, xaxis, yaxis, coef, graySourceList{k});
        time = toc;
    ...
    save(strcat('Output_contours\roi_nocolor_', graySourceList{k}, '_', currDateTime, '.mat'), 'evaluationData');


    %% Pretty Print

    fileID = fopen(strcat('Output_contours\roi_nocolor_', graySourceList{k}, '_', currDateTime, '.txt'), 'w');
    fprintf('----------------------------------------------------------------------------------------\n\n');
